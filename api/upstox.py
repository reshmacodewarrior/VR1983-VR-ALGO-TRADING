...
@router.get("/callback")
async def callback(code: str, current_user: dict = Depends(get_current_user)):
    try:
        ...
        user_id = await store_upstox_connection(profile, token_data, current_user)
...
async def store_upstox_connection(profile: Dict[str, Any], token_data: Dict[str, Any], current_user: dict) -> str:
...
async def get_profile(current_user: dict = Depends(get_current_user)):
    ...
    connection = await get_upstox_connection(current_user.email)
    if not connection:
